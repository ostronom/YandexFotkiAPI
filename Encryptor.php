...
class Encryptor {
	...
	public static function encryptPortion($portion, $size, $n, $e) {
		$padding = ($size & 3) ? (4 - ($size & 3)) : 0;
	...
	public static function encrypt($key, $data){
		...
		$e = self::hex2dec($estr);
		$in_size = strlen($data);
		$data = array_map(function($c){return ord($c);}, self::str2arr($data));
		...
		while ($in_size) {
			...
			$portbuf = array();
			for ($i = 0; $i < $cur_size; $i++) $portbuf[$i] = $data[$i] ^ $prev_crypted[$i];
			$encrypted_portion = self::encryptPortion($portbuf, $cur_size, $n, $e);
